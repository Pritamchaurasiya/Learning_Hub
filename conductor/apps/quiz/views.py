...
from .serializers import (
    QuizListSerializer, QuizDetailSerializer, QuizDetailWithAnswersSerializer,
    QuizAttemptSerializer, QuizAttemptDetailSerializer,
    StartQuizAttemptSerializer, SubmitAnswerSerializer, SubmitQuizSerializer,
...
class QuizAttemptViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        ).select_related('quiz', 'quiz__course').prefetch_related('answers')
    ...
    def start(self, request):
        ...
        if not Enrollment.objects.filter(user=request.user, course=quiz.course).exists():
            return Response(
                {'status': 'error', 'message': 'You must be enrolled in this course to take the quiz'},
...
class QuizResultViewSet(viewsets.ViewSet):
    ...
    def stats(self, request):
        ...
        passed_quizzes = attempts.filter(percentage_score__gte=models.F('quiz__passing_score')).count()
