...
describe('POST /tests/:id/start', () => {
    ...
    it('resumes in-progress attempt → 200', async () => {
        ;
        prismaClient_1.prisma.test.findUnique.mockResolvedValue(TEST);
        prismaClient_1.prisma.testResult.findFirst.mockResolvedValue({
            id: 'existing',
            attemptNumber: 1,
            completedAt: null,
            startedAt: new Date(),
    ...
    it('retries on unique constraint violation (attemptNumber race condition)', async () => {
        ...
        prismaClient_1.prisma.testResult.create = createMock;
        const res = await (0, supertest_1.default)(makeApp()).post('/api/tests/test-1/start');
        expect(res.status).toBe(201);
        expect(res.body.data.attempt_id).toBe('a1');
