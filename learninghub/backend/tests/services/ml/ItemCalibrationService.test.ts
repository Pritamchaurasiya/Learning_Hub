import { ItemCalibrationService } from '../../../src/services/ml/ItemCalibrationService'
...
describe('ItemCalibrationService Suite', () => {
  let service: ItemCalibrationService

  beforeEach(() => {
    jest.clearAllMocks()
    service = new ItemCalibrationService()
  })
  ...
  it('should skip questions with fewer than 5 answers', async () => {
    ...
    expect(conductorClient.calibrateItem).not.toHaveBeenCalled()
  })
  ...
  it('should calibrate item using Conductor ML pipeline when available', async () => {
    ...
    ;(conductorClient.calibrateItem as jest.Mock).mockResolvedValue({
      difficulty: 0.8,
      discrimination: 1.5,
    })
    ...
    expect(conductorClient.calibrateItem).toHaveBeenCalledWith('q-ml', [
      { isCorrect: true },
      { isCorrect: true },
      { isCorrect: false },
      { isCorrect: true },
  ...
  it('should fallback to heuristics when Conductor returns null or invalid difficulty', async () => {
    ...
    ;(conductorClient.calibrateItem as jest.Mock).mockResolvedValue(null)
    mockPrisma.question.update = jest.fn().mockResolvedValue({})
