...
describe('TestEngineService', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        if (mockPrisma.$transaction && mockPrisma.$transaction.mockReset) {
            mockPrisma.$transaction.mockReset();
        }
        mockPrisma.$transaction.mockImplementation(async (callback) => {
            return callback(mockPrisma);
        });
        service = new TestEngineService_1.TestEngineService();
    });
    describe('submitPracticeAnswer', () => {
        ...
        it('should submit correct answer and return feedback', async () => {
            ...
            mockPrisma.testAttemptAnswer.findUnique.mockResolvedValue(null);
            mockPrisma.testAttemptAnswer.upsert.mockResolvedValue({});
            mockPrisma.testResult.update.mockResolvedValue({});
            mockPrisma.topicPerformance.findUnique.mockResolvedValue(null);
            mockPrisma.topicPerformance.upsert.mockResolvedValue({});
        ...
        it('should throw error for non-practice mode', async () => {
            ...
            mockPrisma.question.findUnique.mockResolvedValue(mockTimedQuestion);
            await expect(service.submitPracticeAnswer(mockRequest)).rejects.toThrow('Practice mode is only available for practice tests');
        });
        it('should update existing practice result', async () => {
            ...
            mockPrisma.testAttemptAnswer.findUnique.mockResolvedValue({
                marksObtained: 0,
            });
            mockPrisma.testAttemptAnswer.upsert.mockResolvedValue({});
            mockPrisma.testResult.update.mockResolvedValue({});
            ...
            const result = await service.submitPracticeAnswer(mockRequest);
            expect(result.isCorrect).toBe(true);
            expect(mockPrisma.testResult.update).toHaveBeenCalledWith(expect.objectContaining({
                where: { id: 'result-1' },
        ...
        it('should retry on unique constraint violation (attemptNumber race condition)', async () => {
            ...
            mockPrisma.testResult.upsert = upsertMock;
            const result = await service.submitPracticeAnswer(mockRequest);
            expect(result.isCorrect).toBe(true);
        ...
        it('should update topic performance', async () => {
            ...
            expect(topicPerformanceService.updateForSingleAnswer).toHaveBeenCalledWith(mockRequest.userId, 'math', true);
        });
    });
    describe('getTestQuestions', () => {
        ...
        it('should return questions in order for timed mode', async () => {
            ...
            mockPrisma.test.findUnique.mockResolvedValue(timedTest);
            const result = await service.getTestQuestions('test-1', 'user-1');
            expect(result).toHaveLength(2);
        ...
        it('should throw error for non-existent test', async () => {
            ...
            await expect(service.getTestQuestions('non-existent', 'user-1')).rejects.toThrow('Test not found');
        });
    ...
    describe('getTestAnalytics', () => {
        ...
        it('should return comprehensive analytics', async () => {
            ;
            mockPrisma.testResult.findMany.mockResolvedValue(mockResults);
    ...
    describe('bookmarkQuestion', () => {
        it('should bookmark a question successfully', async () => {
            ...
            mockPrisma.questionBookmark.create.mockResolvedValue({
                id: 'bookmark-1',
                userId: 'user-1',
                questionId: 'question-1',
                notes: 'Review later',
        ...
        it('should throw error for duplicate bookmark', async () => {
            ;
            mockPrisma.questionBookmark.findUnique.mockResolvedValue({
                id: 'existing-bookmark',
            });
            await expect(service.bookmarkQuestion('user-1', 'question-1')).rejects.toThrow('Question already bookmarked');
        });
    });
    describe('getBookmarkedQuestions', () => {
        it('should return bookmarked questions', async () => {
            ...
            mockPrisma.questionBookmark.findMany.mockResolvedValue(mockBookmarks);
    ...
    describe('getAttemptHistory', () => {
        ...
        it('should return attempt history with pagination', async () => {
            ;
            mockPrisma.testResult.findMany.mockResolvedValue(mockAttempts);
        ...
        it('should filter by testId', async () => {
            ...
            expect(mockPrisma.testResult.findMany).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should respect pagination limits', async () => {
            const emptyAttempts = [];
            mockPrisma.testResult.findMany.mockResolvedValue(emptyAttempts);
    ...
    describe('validateTimeRemaining', () => {
        it('should return valid time remaining', async () => {
            ...
            ;
            mockPrisma.testResult.findFirst.mockResolvedValue({
                id: 'attempt-1',
                userId: 'user-1',
                testId: 'test-1',
                status: 'IN_PROGRESS',
    ...
    describe('autoSubmitExpiredTests', () => {
        it('should auto-submit expired tests', async () => {
            ...
            mockPrisma.$queryRaw.mockResolvedValue(expiredAttempts);
