...
const SpacedRepetitionEngine_1 = require("../../src/engines/learning/SpacedRepetitionEngine");
...
describe('SpacedRepetitionEngine Suite', () => {
    let engine;
    beforeEach(() => {
        engine = new SpacedRepetitionEngine_1.SpacedRepetitionEngine();
        jest.clearAllMocks();
    });
    ...
    describe('updateTopicSchedule', () => {
        it('should create a new spaced repetition schedule if none exists', async () => {
            ;
            mockPrisma.spacedRepetitionSchedule.findUnique.mockResolvedValue(null);
            mockPrisma.spacedRepetitionSchedule.create.mockResolvedValue({
                id: 'sched-1',
                userId: 'user-1',
                topicId: 'topic-1',
                intervalDays: 1,
        ...
        it('should update existing schedule when one exists', async () => {
            ;
            mockPrisma.spacedRepetitionSchedule.findUnique.mockResolvedValue({
                id: 'sched-1',
                userId: 'user-1',
                topicId: 'topic-1',
                intervalDays: 1,
            ...
            mockPrisma.spacedRepetitionSchedule.update.mockResolvedValue({});
