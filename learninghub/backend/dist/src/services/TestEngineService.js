...
exports.testEngineService = exports.TestEngineService = void 0;
...
class TestEngineService {
     ...
     */
    async submitPracticeAnswer(req) {
        const question = await prismaClient_1.prisma.question.findUnique({
        ...
        await prismaClient_1.prisma.testResult.upsert({
            where: {
            ...
            create: {
                ...
                passed: isCorrect,
                timeTaken: 0,
                answers: { [req.questionId]: req.selectedOptionId },
                questionResults: [
                    {
     ...
     */
    async getTestQuestions(testId, userId) {
        ...
        const questions = test.mode === 'PRACTICE' ? this.shuffleArray([...test.questions]) : test.questions;
    ...
    async getTestAnalytics(userId) {
        ...
        for (const key of Object.keys(byDifficulty)) {
            ...
            byDifficulty[key].avgScore = Math.round(items.reduce((s, r) => s + r.percentage, 0) / items.length);
        }
     ...
     */
    async bookmarkQuestion(userId, questionId, notes) {
        const existing = await prismaClient_1.prisma.questionBookmark.findUnique({
            where: { userId_questionId: { userId, questionId } },
        });
        if (existing) {
            throw new Error('Question already bookmarked');
        }
        return prismaClient_1.prisma.questionBookmark.create({
            data: {
                userId,
                questionId,
                notes,
    ...
    async getBookmarkedQuestions(userId) {
        return prismaClient_1.prisma.questionBookmark.findMany({
            where: { userId },
            include: {
                question: {
                    select: {
    ...
    async removeBookmark(userId, questionId) {
        await prismaClient_1.prisma.questionBookmark.deleteMany({
    ...
    async getAttemptHistory(userId, filters) {
        ...
        const [attempts, total] = await Promise.all([
            ...
            prismaClient_1.prisma.testResult.count({ where: where }),
        ]);
        return {
     ...
     */
    async validateTimeRemaining(userId, testId) {
     ...
     */
    async autoSubmitExpiredTests() {
        const expiredAttempts = await prismaClient_1.prisma.testResult.findMany({
            where: {
                status: 'IN_PROGRESS',
                test: {
                    timeLimit: { gt: 0 },
        ...
        if (submittedCount > 0) {
            logger_1.default.info(`Auto-submitted ${submittedCount} expired tests`);
        }
        return submittedCount;
    }
    // ─── Private Helpers ───────────────────────────────────────────────────────
    async getExistingAnswers(userId, testId) {
        const result = await prismaClient_1.prisma.testResult.findFirst({
            where: { userId, testId, attemptNumber: 1 },
            select: { answers: true },
        });
        return result?.answers ?? {};
    }
    async updateTopicPerformance(userId, question) {
        for (const tag of question.tags ?? []) {
            await prismaClient_1.prisma.topicPerformance.upsert({
    ...
    async getTopicStatsFromResults(results) {
        const topicMap = {};
...
exports.testEngineService = new TestEngineService();
