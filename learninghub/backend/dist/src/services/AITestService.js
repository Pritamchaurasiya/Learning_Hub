...
exports.aiTestService = exports.AITestService = void 0;
const generative_ai_1 = require("@google/generative-ai");
const prismaClient_1 = require("../prismaClient");
// ─── Prompt Templates ────────────────────────────────────────────────────────
const PROMPT_TEMPLATES = {
    standard: (topic, difficulty, count) => `
...
Respond with ONLY valid JSON (no markdown, no code fences):
{
  "questions": [
    ...
    exam_pattern: (topic, difficulty, count, examName, pattern) => `
...
Respond with ONLY valid JSON (no markdown, no code fences):
{
  "questions": [
    ...
    adaptive: (topic, count, currentLevel) => `
...
class AITestService {
    ...
    async generateTest(req) {
        ...
        if (req.examContext?.examId) {
            const exam = await prismaClient_1.prisma.exam.findUnique({
            ...
            if (exam) {
                ...
                prompt = PROMPT_TEMPLATES.exam_pattern(req.topic, req.difficulty, questionCount, exam.name, patternStr);
            }
            else {
        ...
        const test = await prismaClient_1.prisma.test.create({
            data: {
    ...
    async getUserLevel(userId) {
        const recentResults = await prismaClient_1.prisma.testResult.findMany({
    ...
    validateQuestions(questions, expectedCount) {
        return questions
            ...
            const hasCorrectOption = q.options.some((o) => o.id === q.correct_option_id);
            if (!hasCorrectOption)
                return false;
            return true;
        })
     ...
     */
    async generateWeakAreaTest(userId, count = 10) {
...
exports.aiTestService = new AITestService();
