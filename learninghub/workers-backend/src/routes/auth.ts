import { z } from 'zod'
import { hashPassword, verifyPassword, generateJWT, createJSONResponse, createErrorResponse } from '../utils/helpers'
...
export async function handleAuth(request: Request, env: Env): Promise<Response> {
  ...
  if (path === '/auth/delete-account' && method === 'DELETE') return handleDeleteAccount(request, env)
  ...
  if (path === '/auth/resend-verification' && method === 'POST') return handleResendVerification(request, env)
  ...
  try {
    ...
    return await withDb(env, async (client) => {
      ...
      return createJSONResponse({
        token,
        user: { id: user.id, email: user.email, username: user.username, role: user.role, xp: user.xp, level: user.level },
...
async function handleLogin(request: Request, env: Env): Promise<Response> {
  try {
    ...
    return await withDb(env, async (client) => {
      ...
      await client.query('UPDATE users SET last_active = CURRENT_TIMESTAMP WHERE id = $1', [user.id])

      const token = await generateJWT({ userId: user.id, email: user.email, role: user.role }, env)

      return createJSONResponse({
        token,
        user: { id: user.id, email: user.email, username: user.username, role: user.role, xp: user.xp, level: user.level, streak: user.streak },
  ...
  try {
    ...
    return await withDb(env, async (client) => {
      const user = await queryOne<{ id: string; email: string; role: string }>(
        client, 'SELECT id, email, role FROM users WHERE id = $1', [payload.userId]
      )
      if (!user) return createErrorResponse('User no longer exists', 401)

      const newToken = await generateJWT({ userId: user.id, email: user.email, role: user.role }, env)
...
async function handleLogout(request: Request, env: Env): Promise<Response> {
  try {
    ...
    await withDb(env, async (client) => {
      await client.query('UPDATE users SET last_active = CURRENT_TIMESTAMP WHERE id = $1', [user.userId])
    })
...
async function handleGetProfile(request: Request, env: Env): Promise<Response> {
  try {
    ...
    console.error('Get profile error:', er)
    return createErrorResponse('Internal server error', 500)
  }
}
  ...
  try {
    ...
    return await withDb(env, async (client) => {
      ...
      const result = await queryOne<UserRow>(
        client,
        `UPDATE users SET ${Object.keys(updates).map((k, i) => `${k} = $${i + 1}`).join(', ')} WHERE id = $${Object.keys(updates).length + 1} RETURNING id, email, username, role, xp, level, streak, bio, location, website, avatar_url`,
    ...
    console.error('Update profile error:', er)
    return createErrorResponse('Internal server error', 500)
  }
}
...
async function handleChangePassword(request: Request, env: Env): Promise<Response> {
  try {
    ...
    return await withDb(env, async (client) => {
      const userRow = await queryOne<{ password_hash: string }>(client, 'SELECT password_hash FROM users WHERE id = $1', [user.userId])
      ...
      await client.query('UPDATE users SET password_hash = $1 WHERE id = $2', [newHash, user.userId])

      return createJSONResponse({ message: 'Password changed successfully' })
    })
  } catch (er) {
    console.error('Change password error:', er)
    return createErrorResponse('Internal server error', 500)
  }
}
...
async function handleUploadAvatar(request: Request, env: Env): Promise<Response> {
  try {
    ...
    await withDb(env, async (client) => {
      await client.query('UPDATE users SET avatar_url = $1 WHERE id = $2', [avatar_url, user.userId])
    })

    return createJSONResponse({ data: { avatar_url }, message: 'Avatar updated' })
  } catch (er) {
    console.error('Upload avatar error:', er)
    return createErrorResponse('Internal server error', 500)
  }
}
...
async function handleDeleteAccount(request: Request, env: Env): Promise<Response> {
  try {
    ...
    console.error('Delete account error:', er)
    return createErrorResponse('Internal server error', 500)
  }
}
  ...
  try {
    ...
    return await withDb(env, async (client) => {
      const user = await queryOne<{ id: string; email: string; username: string }>(
        client, 'SELECT id, email, username FROM users WHERE email = $1', [email]
      )
...
async function handleResetPassword(request: Request, env: Env): Promise<Response> {
  try {
    ...
    if (password.length < 8) return createErrorResponse('Password must be at least 8 characters', 400)
  ...
  try {
    ...
    return await withDb(env, async (client) => {
      const user = await queryOne<{ id: string; email: string }>(
        client, 'SELECT id, email FROM users WHERE email = $1', [email]
      )
      if (!user) return createJSONResponse({ message: 'If the email exists, a verification link has been sent.' })
      ...
      return createJSONResponse({ message: 'If the email exists, a verification link has been sent.' })
    })
  } catch (error) {
...
async function handleVerifyEmail(request: Request, env: Env): Promise<Response> {
  try {
    ...
    return await withDb(env, async (client) => {
      const row = await queryOne<{ id: string; user_id: string; email: string; expires_at: string }>(
        client,
