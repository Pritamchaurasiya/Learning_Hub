...
export function createAuthMiddleware(env: Env) {
  return async (request: Request): Promise<{ user: UserContext | null; error: Response | null }> => {
...
export function requireAuth(authMiddleware: ReturnType<typeof createAuthMiddleware>) {
  return async (request: Request, handler: (user: UserContext) => Promise<Response>): Promise<Response> => {
...
export function requireRole(...allowedRoles: UserContext['role'][]) {
  return (user: UserContext): boolean => {
    ...
    if (!hasRole) {
      logger.warn('Role check failed', { userId: user.userId, role: user.role, required: allowedRoles });
    }
    return hasRole;
  };
}
