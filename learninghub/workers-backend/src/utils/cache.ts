 ...
 */
export async function setCache<T>(env: Env, key: string, data: T, ttl: number): Promise<void> {
  try {
 ...
 */
export async function invalidateCachePattern(env: Env, prefix: string): Promise<void> {
  try {
...
export async function getCachedCourseList(
  ...
  const cacheKey = generateCacheKey('courses', `list:${JSON.stringify(filters)}`)
 ...
 */
export async function invalidateCourseCache(env: Env, courseId?: string): Promise<void> {
  if (courseId) {
 ...
 */
export async function invalidateUserEnrollments(env: Env, userId: string): Promise<void> {
