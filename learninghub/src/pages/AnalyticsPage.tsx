...
import { analyticsService, type DashboardStats, type LearningActivity, type SkillProgress } from '../services/analyticsService'
    ...
    >
      <Card className="p-6 border-none shadow-sm hover:shadow-xl transition-all group relative overflow-hidden bg-white dark:bg-gray-900">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-inner`} style={{ backgroundColor: `${color}10` }}>
        ...
        <h3 className="text-3xl font-black text-gray-900 dark:text-white tabular-nums tracking-tighter">{value}</h3>
        <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mt-1">{label}</p>
      </Card>
    </motion.div>
  )
}
   ...
   if (isLoading) {
     return (
       <div className="flex flex-col items-center justify-center min-h-[70vh] space-y-6">
         ...
         <p className="font-bold text-gray-500 animate-pulse tracking-widest uppercase text-[10px]">Processing Metrics...</p>
       </div>
     )
   }
    ...
    <AnimatedPage className="space-y-10 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1">Deep insights into your cognitive growth and mastery</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Clock} label="Total Focus" value={`${stats ? Math.round(stats.total_learning_time / 60) : 0}h`} trend="+12%" color="#3b82f6" delay={0.1} />
        <StatCard icon={BookOpen} label="Course Mastery" value={stats?.completed_courses || 0} color="#10b981" delay={0.2} />
        <StatCard icon={Zap} label="Neural Level" value={stats?.level || 1} color="#f59e0b" delay={0.3} />
        <StatCard icon={Flame} label="Logic Streak" value={`${stats?.current_streak || 0}d`} color="#ef4444" delay={0.4} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Weekly Visualization */}
        <Card className="lg:col-span-2 p-8 border-none shadow-xl bg-white dark:bg-gray-900/50 backdrop-blur-xl relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-10 relative z-10">
            <div>
              ...
              <p className="text-[10px] font-bold text-gray-400 uppercase mt-1">Average: {Math.round(weeklyProgress.reduce((a,b)=>a+b,0)/7 * 10)/10}h / Day</p>
            </div>
          </div>
          <div className="flex items-end justify-between gap-4 h-64 relative z-10">
             {weeklyProgress.map((hours, index) => (
               <div key={index} className="flex-1 flex flex-col items-center gap-3 group">
                  ...
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-tighter">{chartDays[index]}</span>
               </div>
             ))}
          </div>
        </Card>
        ...
        {/* Skills Progress */}
        <Card className="p-8 border-none shadow-xl bg-white dark:bg-gray-900">
            ...
            {skills.slice(0, 5).map((skill, i) => (
              <div key={i} className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs font-black uppercase tracking-wide text-gray-600 dark:text-gray-300">{skill.skill_name}</span>
                  <span className="text-xs font-black tabular-nums text-primary-500">{skill.proficiency_percent}%</span>
                </div>
          ...
          <Button variant="outline" className="w-full mt-10 rounded-2xl font-black uppercase tracking-widest text-[10px] border-2">
            Detailed Skill Tree
          </Button>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
         <Card className="p-8 border-none shadow-xl">
            ...
            <div className="space-y-6">
              {activity.slice(0, 4).map((day, i) => (
                <div key={i} className="flex items-start gap-4 p-4 rounded-[1.5rem] bg-gray-50 dark:bg-gray-800/50 hover:bg-white dark:hover:bg-gray-800 transition-all border border-transparent hover:border-gray-100 dark:hover:border-gray-700">
                   <div className="w-10 h-10 rounded-xl bg-white dark:bg-gray-700 flex flex-col items-center justify-center shadow-sm">
                      <span className="text-[10px] font-black uppercase text-primary-500">{new Date(day.date).toLocaleDateString('en-US', { weekday: 'short' })}</span>
                      <span className="text-xs font-bold">{new Date(day.date).getDate()}</span>
                   </div>
                   <div className="flex-1">
                      <div className="flex justify-between">
                        <p className="text-sm font-bold text-gray-900 dark:text-white">{day.lessons_completed} Lessons Finalized</p>
                        <span className="text-[10px] font-black text-emerald-500">+{day.xp_earned} XP</span>
                      </div>
                      <p className="text-xs text-gray-500 mt-1">{Math.round(day.time_spent / 60 * 10)/10}h Cognitive Session</p>
                   </div>
         ...
         <Card className="p-6 border border-gray-100 dark:border-gray-800 shadow-lg">
            ...
            <div className="space-y-4">
               {[
                  { name: 'System Design Mastery', progress: 85, completed: 17, total: 20, lastAccessed: '2 hours ago' },
                  { name: 'Data Structures Deep Dive', progress: 62, completed: 12, total: 20, lastAccessed: '1 day ago' },
                  { name: 'Advanced Algorithms', progress: 45, completed: 9, total: 20, lastAccessed: '3 days ago' },
               ].map((course, idx) => (
         ...
         <Card className="p-8 border-none shadow-xl bg-primary-600 text-white overflow-hidden relative group">
            ...
            <div className="relative z-10">
               <span className="px-3 py-1 bg-white/20 backdrop-blur-md rounded-lg text-[10px] font-black uppercase tracking-widest">Growth Recommendation</span>
               <h3 className="text-2xl font-black mt-6 mb-4 leading-tight">Master System Design to Reach Level { (stats?.level || 0) + 1 }</h3>
               <p className="text-primary-100/80 mb-10 leading-relaxed font-medium">
                 Based on your 85% proficiency in Backend Architecture, we recommend the Advanced Scalability Lab to accelerate your neural growth.
               </p>
               <div className="flex flex-col sm:flex-row gap-4">
                  <Button className="flex-1 bg-white text-primary-600 border-none font-black shadow-lg">Start Recommendation</Button>
                  <Button variant="outline" className="flex-1 border-white/40 text-white font-black hover:bg-white/10">Compare Peers</Button>
               </div>
            </div>
         </Card>
      </div>
