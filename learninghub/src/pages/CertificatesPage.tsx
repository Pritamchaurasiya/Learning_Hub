...
export default function CertificatesPage() {
  ...
  const connectWallet = async () => {
    try {
      ...
      if (!window.ethereum) {
        addToast({
          message: 'No Web3 wallet detected. Please install MetaMask or similar.',
          type: 'error',
        })
        return
      }
      ...
      if (accounts.length === 0) {
        addToast({ message: 'No accounts found in wallet', type: 'error' })
        return
      }

      await web3Service.updateWallet(accounts[0])
      await fetchData()
              ...
              {web3Profile?.wallet_address ? (
                ...
                <div className="text-center space-y-4 py-2">
                  ...
                  <Button
                    ...
                    className="w-full rounded-xl bg-white text-gray-900 border-none font-black text-[10px] py-4"
                  >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCertificates.map(cert => {
              ...
              return (
                <Card
                  key={cert.id}
                  className="overflow-hidden group hover:shadow-2xl transition-all duration-500 border-none rounded-3xl bg-white dark:bg-gray-900/50"
                >
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <div className="flex items-center justify-between text-xs font-bold text-gray-500 uppercase tracking-wider">
                      <span className="flex items-center gap-1.5">
                        <Calendar className="w-3.5 h-3.5" /> {formatDate(cert.issued_at)}
                      </span>
                      <span className="flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-500" /> Genuine
                      </span>
                    </div>
