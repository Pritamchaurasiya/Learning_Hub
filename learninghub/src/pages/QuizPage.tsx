...
function QuizPage() {
  ...
  useEffect(() => {
    void loadQuiz()
  }, [loadQuiz]) // Fixed: loadQuiz is now properly in deps
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="font-bold text-gray-500 tracking-widest uppercase text-xs">
          Generating Session...
        </p>
      </div>
    )
    ...
    return (
      <AnimatedPage>
        <Card className="max-w-md mx-auto p-10 text-center mt-12 border-none shadow-2xl rounded-[2.5rem]">
          ...
          <Button onClick={loadQuiz} className="w-full py-4 rounded-2xl font-bold">
  ...
  if (!quizId) {
    ...
    return (
      <AnimatedPage className="max-w-6xl mx-auto px-4 pb-12">
        <SEO title="Quiz Discovery" />
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div className="space-y-2">
            <h1 className="text-4xl font-black tracking-tighter">Knowledge Mastery</h1>
            <p className="text-gray-500 dark:text-gray-400 font-medium">
              Challenge yourself with our curated engineering assessments.
            </p>
          </div>
          <div className="relative w-full md:w-96">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 rounded-2xl bg-white dark:bg-gray-900 border-none shadow-xl shadow-gray-200/20 dark:shadow-none focus:ring-2 focus:ring-primary-500 transition-all font-bold"
              value={searchQuery}
        ...
        {filteredQuizzes.length === 0 ? (
          <Card className="p-20 text-center border-none shadow-xl rounded-[3rem]">
            <div className="w-20 h-20 bg-gray-50 dark:bg-gray-800/50 rounded-3xl flex items-center justify-center mx-auto mb-6">
              <Search className="w-10 h-10 text-gray-300" />
            </div>
            <h3 className="text-xl font-black mb-2">No quizzes found</h3>
            <p className="text-gray-500">Try adjusting your search query or check back later.</p>
          </Card>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredQuizzes.map(quiz => (
              ...
              >
                <Card className="p-8 h-full border-none shadow-lg hover:shadow-2xl transition-all duration-500 rounded-[2.5rem] relative overflow-hidden flex flex-col">
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div className="px-3 py-1 bg-gray-50 dark:bg-gray-800 rounded-lg text-[10px] font-black uppercase tracking-widest text-gray-400">
                      {quiz.course_title}
                    </div>
                  </div>

                  <h3 className="text-xl font-black mb-3 tracking-tight group-hover:text-primary-600 transition-colors">
                    {quiz.title}
                  </h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-8 line-clamp-2 leading-relaxed flex-1">
                    {quiz.description}
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-auto">
                    <div className="p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/50">
                      ...
                      <p className="font-bold tabular-nums">{quiz.time_limit}m</p>
                    </div>
                    <div className="p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/50">
                      <div className="flex items-center gap-2 mb-1">
                      ...
                      <p className="font-bold tabular-nums">{quiz.passing_score}%</p>
                    </div>
                  </div>
      ...
      <AnimatedPage className="max-w-3xl mx-auto px-4 pb-12">
        ...
        <Card className="p-8 sm:p-12 text-center mb-10 border-none shadow-2xl rounded-[3rem] relative overflow-hidden">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-10 font-medium">
            {result.passed
              ? "Outstanding! You've mastered these concepts."
          ...
          <div className="grid grid-cols-3 gap-4 mb-10">
            {[
              ...
              <div
                key={stat.label}
                className={`${stat.bg} p-6 rounded-[2rem] border border-transparent`}
              >
                <p
                  className={`text-2xl sm:text-3xl font-black ${stat.color} mb-1 tracking-tighter`}
                >
                  {stat.value}
                </p>
        ...
        <div className="space-y-6">
          ...
          {questions.map((q, index) => {
            ...
            return (
              ...
              >
                <Card
                  className={`p-6 sm:p-8 rounded-[2rem] border shadow-sm ${isCorrect ? 'border-emerald-100 dark:border-emerald-900/20' : 'border-red-100 dark:border-red-900/20'}`}
                >
                  <div className="flex items-start gap-4">
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-1 ${isCorrect ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}
                    >
                      {isCorrect ? (
  ...
  return (
    <AnimatedPage className="max-w-4xl mx-auto px-4 pb-12">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-10">
        ...
        <div
          className={`flex items-center gap-3 px-6 py-4 rounded-[2rem] border-2 transition-all shadow-xl ${timeRemaining < 60 ? 'bg-red-50 border-red-200 text-red-600 animate-pulse' : 'bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700'}`}
        >
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-6">
                    <div
                      className={`w-10 h-10 rounded-2xl border-2 flex items-center justify-center font-black transition-all ${
                        isSelected
                          ? 'bg-primary-500 border-primary-500 text-white'
                          : 'border-gray-200 dark:border-gray-700 text-gray-400 group-hover:border-primary-200'
                      }`}
                    >
