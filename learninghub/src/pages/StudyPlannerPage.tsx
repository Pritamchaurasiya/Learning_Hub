...
import { studyPlannerService, type StudyTask } from '../services/studyPlannerService'
...
export default function StudyPlannerPage() {
  ...
  const toggleTask = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      if (newStatus === 'completed') {
        ...
        await studyPlannerService.updateTask(id, { status: newStatus } as any)
      }
  ...
  const getPriorityStyles = (priority: string) => {
    switch (priority) {
      case 'high': return 'bg-rose-50 text-rose-600 dark:bg-rose-900/20 dark:text-rose-400 border-rose-100 dark:border-rose-800'
      case 'medium': return 'bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400 border-amber-100 dark:border-amber-800'
      case 'low': return 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400 border-emerald-100 dark:border-emerald-800'
  ...
  const goals = [
    { id: '1', title: 'Complete Full-Stack Path', target: '80%', deadline: 'Sun, Apr 26', progress: 65, color: '#3b82f6' },
    { id: '2', title: 'Master 50 DSA Logic Sets', target: '50', deadline: 'Wed, Apr 29', progress: 42, color: '#8b5cf6' }
  ]
    ...
    <AnimatedPage className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
           ...
           <div>
              <h1 className="text-3xl font-black tracking-tight leading-none text-gray-900 dark:text-white">Strategy Planner</h1>
              <p className="text-gray-500 font-medium mt-2">Orchestrate your cognitive trajectory</p>
           </div>
        </div>
        <Button size="lg" className="rounded-2xl font-black shadow-xl shadow-primary-500/20" leftIcon={<Plus className="w-5 h-5" />}>
          New Study Protocol
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-1 space-y-6">
            <Card className="p-8 border-none shadow-xl bg-gradient-to-br from-orange-500 to-rose-600 text-white overflow-hidden relative group">
               ...
               <div className="relative z-10">
                  <p className="text-[10px] font-black uppercase tracking-widest opacity-80">Weekly Momentum</p>
                  <h3 className="text-4xl font-black mt-2 mb-1">12.5h <span className="text-xl opacity-60">/ 20h</span></h3>
                  ...
                  {goals.map(goal => (
                    <div key={goal.id} className="space-y-4">
                       <div className="flex justify-between items-start">
                          <div>
                             <p className="text-sm font-bold text-gray-900 dark:text-white leading-none">{goal.title}</p>
                             <p className="text-[10px] font-bold text-gray-400 uppercase mt-2">Expires: {goal.deadline}</p>
                          </div>
                          <span className="text-xs font-black tabular-nums text-primary-500">{goal.progress}%</span>
                       </div>
         ...
         <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
               {isLoading ? (
                 [1,2,3].map(i => <div key={i} className="h-24 w-full bg-gray-100 dark:bg-gray-800/50 animate-pulse rounded-[2rem]" />)
               ) : tasks.length === 0 ? (
                 <Card className="p-20 text-center border-none shadow-sm bg-white dark:bg-gray-900/30">
                    ...
                    <h3 className="text-xl font-black text-gray-900 dark:text-white mb-2">Neural Workspace Clear</h3>
                    <p className="text-gray-500 text-sm max-w-xs mx-auto leading-relaxed">
                      All protocols executed. Initialize new study tasks to continue trajectory.
                    </p>
                    <Button variant="outline" className="mt-8 rounded-xl font-bold border-2">Add Protocol</Button>
                 </Card>
               ) : (
                           ...
                           <div className="flex items-center gap-6">
                              ...
                              <div className="flex-1 min-w-0">
                                 <div className="flex items-start justify-between gap-4">
                                    <div>
                                       <h4 className={`text-lg font-bold tracking-tight truncate ${task.status === 'completed' ? 'line-through text-gray-400' : 'text-gray-900 dark:text-white'}`}>
                                         {task.title}
                                       </h4>
                                       {task.course && (
                                 ...
                                 <div className="flex items-center gap-6 mt-4">
                                    <span className={`px-2.5 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-widest border ${getPriorityStyles(task.priority)}`}>
                                       {task.priority}
                                    </span>
