...
export const CourseCard = React.memo(({ 
  ...
  const duration = course.duration || (course.estimatedTime ? `${course.estimatedTime} min` : 'N/A');
  const level = course.level || course.difficulty || 'All Levels';

  return (
    <motion.div
      className={cn(
        "card p-5 cursor-pointer group focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 border border-gray-100 dark:border-gray-800 flex flex-col h-full",
        className
      )}
    ...
    >
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex gap-1.5 items-center">
          ...
          {isBookmarked && (
            <span className="text-primary-500 text-sm" title="Bookmarked">🔖</span>
          )}
        </div>
      </div>
